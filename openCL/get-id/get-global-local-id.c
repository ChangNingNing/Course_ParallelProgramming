...
int main(int argc, char *argv[]){
	...
	assert(status == CL_SUCCESS);
	status = clSetKernelArg(kernel, 1, sizeof(cl_mem), (void*)&bufferLocalId);
	assert(status == CL_SUCCESS);
	...
	status = clEnqueueNDRangeKernel(commandQueue, kernel, 2, NULL, globalDim, localDim, 0, NULL, NULL);
	assert(status == CL_SUCCESS);
...
//	clFinish(commandQueue);
	...
	assert(status == CL_SUCCESS);
	status = clEnqueueReadBuffer(commandQueue, bufferLocalId, CL_TRUE, 0, 2*N*N*sizeof(cl_uint), localId, 0, NULL, NULL);
	assert(status == CL_SUCCESS);
	...
	clReleaseMemObject(bufferLocalId);
	return 0;
}
