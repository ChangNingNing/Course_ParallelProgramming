...
int main(int argc, char *argv[]){
	...
	/* get command queue */
	cl_command_queue commandQueue = clCreateCommandQueue(context, GPU[0], CL_QUEUE_PROFILING_ENABLE, &status);
	assert(status == CL_SUCCESS);
	...
	cl_event event;
	status = clEnqueueNDRangeKernel(commandQueue, kernel, 2, NULL, globalThreads, localThreads, 0, NULL, &event);
	assert(status == CL_SUCCESS);
	...
	status = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_QUEUED, sizeof(cl_ulong), &timeEnterQueue, NULL);
	assert(status == CL_SUCCESS);
	status = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_SUBMIT, sizeof(cl_ulong), &timeSubmit, NULL);
	assert(status == CL_SUCCESS);
	status = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &timeStart, NULL);
	assert(status == CL_SUCCESS);
	status = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &timeEnd, NULL);
	assert(status == CL_SUCCESS);

	/* print time */
	printf("kernel queue time %f seconds\n", (timeSubmit - timeEnterQueue) / 1000000000.0);
